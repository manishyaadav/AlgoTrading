...
- Persists `PriorClose` alongside Pivot Central Range now, not just `Pivot`/`TopCentral`/`BottomCentral`/`Width` — it was already computed to seed PCR itself, just discarded before; kept because the deployed strategy's own `TradingSessionRules` gate needs yesterday's raw close to evaluate "0.0038 × Closing Price (Previous)" for real (see the Rule Engine bullet right below).
...
**Dashboard "Rule Engine" page + `StrategyService`'s `GET /api/strategies/{id}/rule-status`** ✅ shipped (not one of the numbered sections below — a related but distinct capability that consumes the indicator state 2e produces, rather than being part of building it): read-only visualization of a deployed strategy's rule tree, evaluated live against whatever Redis state actually exists — see the "Strategy execution engine" entry under [Parked](#4-parked--deferred-revisit-when-reached) for the important distinction between this and a real execution engine, and `StrategyService/README.md` / `design-system/algotrading-dashboard/pages/rule-engine.md` for full detail.
...
- **Strategy execution engine** — still parked: nothing in this codebase generates a real entry/exit trigger or places an order. What *does* now exist, and is a different, smaller thing: `StrategyService`'s `GET /api/strategies/{id}/rule-status` (the dashboard's new **Rule Engine** page) reads a deployed strategy's `TradingRule` tree and evaluates each condition it can against live Redis state — read-only, no side effects, nothing written back. It's a visualization of what the rules would currently evaluate to, not the engine itself; building here was explicitly *not* "design the execution engine," see `design-system/algotrading-dashboard/pages/rule-engine.md` for the exact scope line drawn (which rule shapes resolve today vs. stay honestly "unknown") and `StrategyService/README.md`'s own section for the same.
- **Portfolio / position management** — deferred explicitly until triggers are actually being generated. Correction to an earlier version of this doc: `PortfolioService` does **not** exist in this repo — confirmed via full repo search while building the Rule Engine page above (no directory, no git history of one ever existing). What does exist is three empty SignalR hub stubs (`PortfolioHub`/`OrderHub`/`RiskHub` in `Helpers/SignalRServer`) with no domain logic — placeholders for names this system's design anticipates, not a built service. The Rule Engine page's "In a position?" gate is drawn as a permanent, honest placeholder for exactly this reason — there's nothing real to check yet.
