...
namespace OHLCFunctionApp
{
    ...
    // today's daily file's data rows and bulk-appends them into the monthly file in one round trip
    // via AppendManyAsync — not one AppendAsync call per row, which would mean ~375 sequential
    // whole-file re-uploads against the (already large, cumulative) monthly blob.
    //
    // Known limitation, matching LiveCandlePersistenceFunction's own risk posture: no dedup guard
    // against a redelivered Close event re-merging the same day's rows a second time. The daily file
    // itself is left in place after a successful merge — purging it is a separate, not-yet-implemented
    // concern (see this project's README).
            ...
            foreach (var exchange in Exchanges)
            {
                foreach (var ticker in Tickers)
                {
                    ...
                    try
                    {
                        ...
                        await _appendStrategy.AppendManyAsync(container, monthlyPath, BlobPathHelper.HeaderLine, dataRows, _logger);
                        _logger.LogInformation($"DailyToMonthlyMergeFunction: merged {dataRows.Count} row(s) from {dailyPath} into {monthlyPath}");
                    }
                    catch (Exception ex)
                    {
