...
namespace AggregatorFunctions.TimeFrameAggregator.MockTimeframeAggregator
{
    public class MockAggregation5MinutesFunction
    {
        private readonly ILogger<MockAggregation5MinutesFunction> _logger;
        private static Dictionary<string, List<DataIngestionMinDataEvent>> _bufferData = new Dictionary<string, List<DataIngestionMinDataEvent>>();
        private static List<Bucket> _bucket = new List<Bucket>();
        private static int _timeframe = 5;
        private readonly IProducer<string, string> _producer;
        private static string _producerTopicName = $"mock-aggregation-ohlc-{_timeframe}min-topic";

        public MockAggregation5MinutesFunction(ILogger<MockAggregation5MinutesFunction> logger,
        ...
        [Function("MockAggregator5MFunction")]
        public async Task Run(
               [KafkaTrigger("%KAFKA_BROKER_URL%",
                  "mock-dataingestion-ohlc-topic",
                  AuthenticationMode = BrokerAuthenticationMode.Plain,
                  ConsumerGroup = "mock-dataingestion-5min-aggregator")] string eventDataJson, FunctionContext context)
        {
            ...
            try
            {
                ...
                if (eventValue != null)
                {
                    ...
                    if (eventValue != null)
                    {
                        ...
                        {
                            _bufferData[eventValue.Ticker] = new List<DataIngestionMinDataEvent>();
                        }
                        ...
                        DateTime currentTime = eventValue.WindowsStartTime;
                        DateTime tmpBucketStart = currentTime.AddMinutes(-(currentTime.Minute % _timeframe));
        ...
        private TimeFrameAggregationEvent? CalculateMinAggregation(int timeframe, List<DataIngestionMinDataEvent> data)
        {
            ...
            if (first != null && last != null)
            {
                ...
                return new TimeFrameAggregationEvent()
                {
                    ...
                    Producer = "mock.aggregator.service",
