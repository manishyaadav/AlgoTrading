...
namespace AggregatorFunctions.Indicators
{
    ...
    // Aggregation75MinutesFunction already publishes to (not the raw 1-min topic), and hands each
    ...
    public class IndicatorDispatcher75MinFunction
    {
        private readonly IndicatorDispatcher _dispatcher;

        public IndicatorDispatcher75MinFunction(ILogger<IndicatorDispatcher75MinFunction> logger, IProducer<string, string> producer, RedisHelper redisHelper)
        {
            _dispatcher = new IndicatorDispatcher(logger, producer, redisHelper);
        }

        [Function("IndicatorDispatcher75MinFunction")]
        public async Task Run(
            [KafkaTrigger("%KAFKA_BROKER_URL%",
                "live-aggregation-ohlc-75min-topic",
                AuthenticationMode = BrokerAuthenticationMode.Plain,
                ConsumerGroup = "live-indicator-75min-dispatcher")] string eventDataJson,
            FunctionContext context)
        {
            var logger = context.GetLogger("IndicatorDispatcher75MinFunction");
            try
            {
                ...
                await _dispatcher.DispatchAsync(bar, 75);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Indicator dispatch error (75min)");
            }
        }
    }
}
