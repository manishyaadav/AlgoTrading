...
namespace AggregatorFunctions.Indicators
{
    ...
    // Aggregation10MinutesFunction already publishes to (not the raw 1-min topic), and hands each
    ...
    public class IndicatorDispatcher10MinFunction
    {
        private readonly IndicatorDispatcher _dispatcher;

        public IndicatorDispatcher10MinFunction(ILogger<IndicatorDispatcher10MinFunction> logger, IProducer<string, string> producer, RedisHelper redisHelper)
        {
            _dispatcher = new IndicatorDispatcher(logger, producer, redisHelper);
        }

        [Function("IndicatorDispatcher10MinFunction")]
        public async Task Run(
            [KafkaTrigger("%KAFKA_BROKER_URL%",
                "live-aggregation-ohlc-10min-topic",
                AuthenticationMode = BrokerAuthenticationMode.Plain,
                ConsumerGroup = "live-indicator-10min-dispatcher")] string eventDataJson,
            FunctionContext context)
        {
            var logger = context.GetLogger("IndicatorDispatcher10MinFunction");
            try
            {
                ...
                await _dispatcher.DispatchAsync(bar, 10);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Indicator dispatch error (10min)");
            }
        }
    }
}
