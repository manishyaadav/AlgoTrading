...
- A status badge — **not** just raw count vs. total, since a session isn't over until 3:30. It's count vs. *how many should have landed by now*, computed server-side from elapsed time since 9:15, as an **absolute bucket gap** rather than a ratio: **Pending** (before market open, nothing to be behind on yet), **On Track** (≤1 bucket behind — routine processing lag, not a problem), **Behind** (2-3 buckets behind), **Behind / No Data** (4+ buckets behind, or nothing has arrived at all this session). A ratio (e.g. "≥90% of expected") made low-bucket-count timeframes wildly over-sensitive — 75-min only has 5 buckets in a session, so the same one-bucket lag that's invisible on 1-min (375 buckets) used to swing 75-min straight to amber.

  Ingested bars stay **green regardless of this status** — the status only colors the shortfall marker (the gap between what arrived and what's expected) and the badge text, never the bars that already landed. See [design-system/algotrading-dashboard/pages/data.md](../../../design-system/algotrading-dashboard/pages/data.md#status-colour-never-repaints-arrived-data).
